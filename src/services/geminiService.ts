...
export const generateTopicQuestions = async (keyword: string): Promise<string[]> => {
...
export const optimizeAndExtract = async (transcription: string): Promise<{ optimized: string; chunks: Chunk[] }> => {
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
...
export const autoCompleteChunk = async (partial: { original?: string; translation?: string }): Promise<Partial<Chunk>> => {
